...
var allSignalManifest = []ManifestEntry{
	...
	},
	{
		...
		Domain: models.CategoryAI, Status: StatusPlanned,
		Title:           "Prompt Changed Without Version Bump",
		Description:     "Prompt content changed between commits without a corresponding version field bump or filename version suffix.",
		Remediation:     "Bump the prompt's version metadata or rename the file with a new version suffix before merging.",
		DefaultSeverity: models.SeverityMedium,
		ConfidenceMin:   0.85, ConfidenceMax: 0.95,
		EvidenceSources: []string{"structural-pattern", "graph-traversal"},
		RuleID:          "TER-AI-101", RuleURI: "docs/rules/ai/prompt-versioning.md",
		PromotionPlan:   "0.2 — git-blame-driven detector lands with the prompt-surface expansion.",
	},
	{
	...
	},
	{
		...
		Description:     "Few-shot examples in a prompt overlap with the eval test set, inflating reported scores.",
		Remediation:     "De-duplicate or hold out the contaminated examples; re-run the eval.",
		DefaultSeverity: models.SeverityMedium,
		ConfidenceMin:   0.7, ConfidenceMax: 0.9,
		EvidenceSources: []string{"graph-traversal"},
		RuleID:          "TER-AI-109", RuleURI: "docs/rules/ai/few-shot-contamination.md",
		PromotionPlan:   "0.2 — string-similarity detector across the prompt-surface inventory.",
	},
	{
		Type: SignalAIEmbeddingModelChange, ConstName: "SignalAIEmbeddingModelChange",
		Domain: models.CategoryAI, Status: StatusPlanned,
		Title:           "Embedding Model Swap Without Re-Evaluation",
		Description:     "An embedding model name or version changed in the codebase without a corresponding RAG re-evaluation in the gauntlet.",
		Remediation:     "Run the RAG eval suite against the new embeddings; commit the updated baseline.",
		DefaultSeverity: models.SeverityMedium,
		ConfidenceMin:   0.85, ConfidenceMax: 0.95,
		EvidenceSources: []string{"structural-pattern", "graph-traversal"},
		RuleID:          "TER-AI-110", RuleURI: "docs/rules/ai/embedding-model-change.md",
		PromotionPlan:   "0.2 — git-blame detector. Promotes to stable once the embedding-aware surface scanner ships.",
	},
	{
