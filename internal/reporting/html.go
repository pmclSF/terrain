...
// It embeds NO wall-clock timestamp, so two runs on the same analysis produce
// byte-identical HTML (a team snapshotting/hosting the report doesn't get a diff
// on every run) — matching the determinism of the JSON/SARIF/text artifacts.
func RenderAnalyzeHTML(w io.Writer, report *analyze.Report) error {
	return htmlTmpl.Execute(w, htmlData{Report: report})
}
...
<title>Terrain Analysis{{if .Report.Repository.Name}} — {{.Report.Repository.Name}}{{end}}</title>
<style>
...
{{if .Report.Repository.Name}}<div class="subtitle">{{.Report.Repository.Name}}</div>{{end}}
...
<div class="footer">
  Generated by <a href="https://github.com/pmclSF/terrain">Terrain</a>
</div>
