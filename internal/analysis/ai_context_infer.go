...
var (
	...
	// AI instruction markers tightened 2026-05-11 against hand-labeled
	// aiPromptVersioning corpus: the old `do not` and `instructions?:`
	// patterns matched every "DO NOT EDIT — generated by …" header in
	// Flutter/Dart/Go/Ruby/Ansible codegen templates. Hand-validated
	// 25-sample review showed 23/25 FPs across codegen templates with
	// no LLM use whatsoever. Replaced with patterns that almost only
	// appear in actual LLM prompts (second-person directives,
	// chat-role markers, and the standalone `system: ` / `user: ` line
	// markers that aren't preceded by a YAML field path).
	aiInstructionMarkers = regexp.MustCompile(`(?i)\b(you are a|you are an|as an ai|as a (?:helpful|careful) (?:assistant|agent)|always respond|your (role|task|job) is to|you must (?:always|never)|respond (?:only|with) (?:as|json|the following))\b`)
	// aiRoleMarkers detects chat-style role declarations that appear
	// in prompt templates (system / user / assistant / human at the
	// start of a line). YAML config files using `system:` or
	// `instructions:` as field names won't match because they're
	// followed by a config payload, not by an LLM-style instruction.
	aiRoleMarkers = regexp.MustCompile(`(?m)^[\s\-]*(?:system|user|assistant|human)\s*[:>]\s*(?:you|please|here|the user)`)
)
...
func detectTemplateFiles(root string, existingIDs map[string]bool) []models.CodeSurface {
	...
	_ = filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		// Either an LLM instruction marker OR an LLM role marker
		// (system:/user:/assistant: followed by an instruction).
		if !aiInstructionMarkers.MatchString(src) && !aiRoleMarkers.MatchString(src) {
			return nil
		}
