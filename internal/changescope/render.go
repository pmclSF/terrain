...
// RenderPRSummaryMarkdown renders the PR-comment surface.
//
// Contract:
//
//	P1. The verdict/posture header and the provenance footer always render.
//	P2. The BLOCK/GATE/WATCH/NOTE legend renders iff there are findings to
//	    label (directRisk / indirectRisk / existingDebt non-empty).
//	P3. Severity = disclosure: history-demoted findings render [WATCH] not
//	    [GATE]; an all-clear PR renders the "All clear" callout.
func RenderPRSummaryMarkdown(w io.Writer, pr *PRAnalysis) {
	...
	// Merge recommendation (the explanation subtitle; the verdict itself is
	// the Swiss verdict line below).
	_, mergeExpl := mergeRecommendationForPR(pr.PostureBand, findings, pr.BlockingCount)

	// --- Header — Swiss verdict (email-survivable first line) + subtitle. ---
	// The verdict carries the result even when a notification email flattens
	// everything below it. New risks in changed code are the blocking count.
	line("**Terrain** · pre-flight")
	line("")
	// The verdict reflects what actually fails the merge gate (pr.BlockingCount),
	// not every direct-risk finding — so it never claims "N findings block this
	// merge" on a PR that passes.
	line("%s", render.VerdictLine(pr.BlockingCount))
	line("")
	if mergeExpl != "" {
		line("> %s", mergeExpl)
		line("")
	...
	// Severity-label legend — shown once, only when findings will be
	// labeled below, so a first-time reader understands BLOCK/GATE/WATCH/
	// NOTE without leaving the comment.
	if len(directRisk) > 0 || len(indirectRisk) > 0 || len(existingDebt) > 0 {
		line("%s", render.SeverityLegend())
		line("")
	}
	...
	line("<sub>%s · [Terrain](https://github.com/pmclSF/terrain) · `terrain pr --json`</sub>", render.ProvenanceFooter(""))
}
...
// mergeRecommendationForPR derives the recommendation and subtitle so they
// can never contradict the merge gate: when the gate blocks (blockingCount
// > 0) the recommendation is forced to a blocked verdict regardless of the
// change-scoped posture band, because off-diff Criticals / alwaysGate findings
// and the --fail-on threshold feed BlockingCount but not the posture band.
func mergeRecommendationForPR(postureBand string, findings []ChangeScopedFinding, blockingCount int) (recommendation, explanation string) {
	if blockingCount > 0 {
		noun := "finding"
		if blockingCount != 1 {
			noun = "findings"
		}
		return "Merge blocked", fmt.Sprintf("%d %s must be resolved before this can merge.", blockingCount, noun)
	}
	return MergeRecommendation(postureBand, findings)
}
...
func RenderPRCommentConcise(w io.Writer, pr *PRAnalysis) {
	...
	findings := DeduplicateFindings(pr.NewFindings)
	mergeRec, _ := mergeRecommendationForPR(pr.PostureBand, findings, pr.BlockingCount)
...
func RenderCIAnnotation(w io.Writer, pr *PRAnalysis) {
	...
	for _, f := range findings {
		...
		fmt.Fprintf(w, "::%s file=%s::%s\n", level, escapeWorkflowProp(f.Path), escapeWorkflowData(f.Explanation))
	}
}

// escapeWorkflowData escapes message text for a GitHub Actions workflow
// command so a newline or '%' in detector text can't split or corrupt the
// annotation.
func escapeWorkflowData(s string) string {
	return strings.NewReplacer("%", "%25", "\r", "%0D", "\n", "%0A").Replace(s)
}

// escapeWorkflowProp escapes a workflow-command property value, which
// additionally requires ',' and ':' to be escaped.
func escapeWorkflowProp(s string) string {
	return strings.NewReplacer("%", "%25", "\r", "%0D", "\n", "%0A", ",", "%2C", ":", "%3A").Replace(s)
}
...
// formatSuitePercent renders selected/total as a percentage. A
// non-zero selection below 1% renders as "<1%" (rather than
// flooring to "0%"), and values >=1% render as integer percentages,
// so a small but non-empty selection is never shown as "0%".
