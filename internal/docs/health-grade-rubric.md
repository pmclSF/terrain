...
When a larger labeled corpus is available, threshold constants will be re-derived from corpus distribution rather than gut-feel, the grading function may incorporate weighted severity, and the cascade may collapse to a smaller table once which boundaries matter empirically is known. The number of grades and their letter labels are stable; public CI gates that key on `healthGrade == "D"` will continue to work.
