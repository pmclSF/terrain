...
## User-facing labels (PR-comment surface)

The internal severity model uses CVSS-style labels (Critical / High / Medium / Low / Info) and clause IDs (`sev-critical-001`, etc.) that are stable forever and used in JSON output for SOC and audit consumers.

The PR-comment surface and IDE diagnostics use a developer-mental-model label set:

| Internal severity | User-facing label | What the engineer does |
|---|---|---|
| Critical | **BLOCK** | Don't merge — production reach |
| High | **GATE** | Fix in this PR or write a dismiss reason |
| Medium | **WATCH** | Visible, doesn't block; track |
| Low | **NOTE** | Footer collapsed; informational |
| Info | _(dropped from PR surface)_ | Stays in `terrain analyze` JSON only |

The clause IDs remain stable and are never renamed. The mapping above is the surface-rendering rule, not a change to the severity model. Renderer logic lives in the PR-comment template and IDE diagnostics layer.
