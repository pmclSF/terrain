...
The catalog detectors and the verdict engine may partially converge in a future release: catalog detectors producing typed atoms that feed the verdict engine's composer, so a single `terrain ai findings` call surfaces both shape-class and gradient findings in one chain.
