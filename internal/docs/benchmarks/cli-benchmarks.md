...
go run ./cmd/internal/terrain-bench/

# Single repo
go run ./cmd/internal/terrain-bench/ --repo terrain

# Single command across all repos
go run ./cmd/internal/terrain-bench/ --command analyze

# Auto-discover repos from a directory
go run ./cmd/internal/terrain-bench/ --discover /path/to/repos

# Use a specific terrain binary (skip auto-rebuild)
go run ./cmd/internal/terrain-bench/ --terrain /usr/local/bin/terrain

# Skip rebuilding and use an existing binary
go run ./cmd/internal/terrain-bench/ --rebuild=false

# Or build the binary first
go build -o terrain-bench ./cmd/internal/terrain-bench/
./terrain-bench --repo terrain --command analyze

# Shell wrapper (delegates to cmd/internal/terrain-bench/)
...
├── run-cli-benchmarks.sh  # Shell wrapper (delegates to cmd/internal/terrain-bench/)
