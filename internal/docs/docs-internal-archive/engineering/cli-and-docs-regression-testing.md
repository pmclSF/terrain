...
Terrain's CLI regression tests verify that the compiled binary behaves
...
the `terrain` binary directly.

## How CLI Tests Work

Each test follows the same pattern:

1. Build the binary from `cmd/terrain/` using `go build`.
...
3. If the exit code is wrong, check command registration in `cmd/terrain/`.
...
- `internal/docs/engineering/e2e-scenario-testing.md` for pipeline-level E2E tests
