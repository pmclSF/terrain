...
Property-based tests (`fast-check`, `hypothesis`, `quickcheck`) are classified as `unit` today even though they're a meaningfully different shape. Adding `property-based` as a first-class type is future work — the framework type enum already has the slot (`FrameworkTypePropertyBased`); the inference rules don't.

### Contract tests vs. integration tests

Pact and Dredd both surface as `integration` today even though contract testing is a distinct discipline. Splitting them would require reading the specific contract artifact (pact JSON, OpenAPI spec) — future work.
