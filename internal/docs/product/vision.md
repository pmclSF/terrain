...
0.2.0 is the foundational release: "see clearly + gate progressively." Three pillars at parity floors (Gate ≥ 3, Understand ≥ 3, Align ≥ 3 soft); suppressions; finding IDs; `--new-findings-only`; AI risk subdivision into inventory/hygiene/regression; multi-repo manifest; alignment-first migration framing; per-detector "known false positives" docs.

Gate floor=3 reflects recall-anchored synthetic calibration. Real-repo precision-floor work, expanded detector coverage, and broader ecosystem integrations continue in future releases. Marketing maturity tracks engineering maturity per the parity gate.
