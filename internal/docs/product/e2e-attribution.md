...
These are out of scope today. They are documented up front so adopters don't infer guarantees that aren't there.
...
Direction of likely future investment includes coverage-artifact ingestion for e2e (LCOV / Istanbul from `playwright test --coverage`), router config parsing for major frameworks, and opt-in selector-to-component mapping via `.terrain/e2e-config.yaml`. None of these are in scope for 0.2; the honest carve-out documented above is the 0.2 contract.
