/**
 * Shared helper for nose2 → pytest fixture tests.
 *
...
export async function getPipeline() {
  ...
  const { default: nose2Def } = await import('../../../src/languages/python/frameworks/nose2.js');
  const { default: pytestDef } = await import('../../../src/languages/python/frameworks/pytest.js');

  registry.register(nose2Def);
 ...
 * @param {string} id - Test case ID (e.g., 'ASSERT-001')
...
export async function runFixture(dir, id, options = {}) {
  ...
  const result = await pipe.convert(input, 'nose2', 'pytest');
