/**
 * Shared helper for unittest → pytest fixture tests.
 *
...
export async function getPipeline() {
  ...
  const { default: unittestDef } = await import('../../../src/languages/python/frameworks/unittest_fw.js');
  const { default: pytestDef } = await import('../../../src/languages/python/frameworks/pytest.js');

  registry.register(unittestDef);
  registry.register(pytestDef);
...
export async function runFixture(dir, id, options = {}) {
  ...
  const result = await pipe.convert(input, 'unittest', 'pytest');
