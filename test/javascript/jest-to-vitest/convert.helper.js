/**
 * Shared helper for Jest→Vitest fixture tests.
 *
...
export async function getPipeline() {
  ...
  // Dynamic imports so this file can be loaded before frameworks are implemented
  const { default: jestDef } = await import('../../../src/languages/javascript/frameworks/jest.js');
  const { default: vitestDef } = await import('../../../src/languages/javascript/frameworks/vitest.js');

  registry.register(jestDef);
  registry.register(vitestDef);
...
export async function runFixture(dir, id, options = {}) {
  ...
  const result = await pipe.convert(input, 'jest', 'vitest');
