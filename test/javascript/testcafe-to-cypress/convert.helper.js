/**
 * Shared helper for TestCafe→Cypress fixture tests.
 *
...
export async function getPipeline() {
  ...
  const { default: testcafeDef } = await import('../../../src/languages/javascript/frameworks/testcafe.js');
  const { default: cypressDef } = await import('../../../src/languages/javascript/frameworks/cypress.js');

  registry.register(testcafeDef);
...
export async function runFixture(dir, id, options = {}) {
  ...
  const result = await pipe.convert(input, 'testcafe', 'cypress');
