/**
 * Shared helper for WebdriverIO→Cypress fixture tests.
 *
...
export async function getPipeline() {
  ...
  const { default: wdioDef } = await import('../../../src/languages/javascript/frameworks/webdriverio.js');
  const { default: cypressDef } = await import('../../../src/languages/javascript/frameworks/cypress.js');

  registry.register(wdioDef);
  registry.register(cypressDef);
...
export async function runFixture(dir, id, options = {}) {
  ...
  const result = await pipe.convert(input, 'webdriverio', 'cypress');
