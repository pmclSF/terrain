...
# Conversion Process (Legacy Converter)

## Overview

Terrain converts test files between frameworks using a multi-step pipeline:
...
5. **Report** - generate TERRAIN-TODO markers for patterns that need manual review
...
When a pattern cannot be automatically converted, Terrain inserts a marker:

```javascript
// TERRAIN-TODO: cy.intercept() has no direct equivalent in Vitest
// Original: cy.intercept('POST', '/api/login').as('login')
```

Common reasons for TERRAIN-TODO markers:
