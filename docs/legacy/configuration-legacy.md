...
# Configuration (Legacy Converter)

Terrain is configured through CLI flags and programmatic API options. There is no configuration file format.
...
```javascript
import { ConverterFactory } from 'terrain-converter/core';
...
```javascript
import { convertFile } from 'terrain-converter';
...
```javascript
import { convertRepository } from 'terrain-converter';
...
```javascript
import { FRAMEWORKS } from 'terrain-converter/core';
