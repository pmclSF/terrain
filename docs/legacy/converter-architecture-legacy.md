...
├── comment: string | null                // TERRAIN-TODO marker if unconvertible
...
> **Implementation status (legacy converter):** The pipeline stages below are wired end-to-end
...
│              │  - Marks unconvertible nodes with TERRAIN-TODO comments
...
> **Implementation status (legacy converter):** In practice, all 20 pipeline directions
...
> **Implementation status (legacy converter):** tree-sitter is not yet integrated.
...
Weight assignments:
  ...
  Unconvertible (TERRAIN-TODO)               = 0  (counts against totalWeight)
```
...
│ Unconvertible:  2 (TERRAIN-TODO markers added)   │
...
| 70–89% | Medium | Some patterns need manual review. TERRAIN-TODO markers present. |
...
```
// TERRAIN-TODO [UNCONVERTIBLE-XXX]: <description>
...
**JavaScript:**
```javascript
// TERRAIN-TODO [UNCONVERTIBLE-001]: Jest snapshot testing has no Vitest equivalent
...
**Python:**
```python
# TERRAIN-TODO [UNCONVERTIBLE-003]: pytest conftest.py fixture hierarchy
...
**Ruby:**
```ruby
# TERRAIN-TODO [UNCONVERTIBLE-004]: RSpec shared_examples cannot be
...
**Java:**
```java
// TERRAIN-TODO [UNCONVERTIBLE-006]: JUnit 4 @Rule requires manual
...
Default behavior: **skip and warn**. Unrecognized code passes through unchanged with a `TERRAIN-WARNING` comment above it. The file still converts — it just has lower confidence.
...
| Commented-out code | Preserved with `TERRAIN-WARNING: commented-out code` |
...
```bash
terrain detect ./tests/test_auth.py
# Output: pytest (confidence: 92%), unittest (confidence: 8%)

terrain detect ./tests/AuthTest.java
# Output: JUnit 4 (confidence: 88%), TestNG (confidence: 12%)

terrain detect ./tests/auth.test.js
...
```bash
# Existing (unchanged)
terrain convert ./tests/ --from cypress --to playwright -o ./output/

# New directions
terrain convert ./tests/ --from jest --to vitest -o ./output/
terrain convert ./tests/ --from pytest --to unittest -o ./output/
terrain convert ./tests/ --from rspec --to minitest -o ./output/
terrain convert ./tests/ --from junit4 --to junit5 -o ./output/
terrain convert ./tests/ --from mocha --to jest -o ./output/
```

### Shorthand Commands

```bash
terrain jest2vt <source>       # Jest → Vitest
terrain vt2jest <source>       # Vitest → Jest
terrain mocha2jest <source>    # Mocha+Chai → Jest
terrain j4toj5 <source>        # JUnit 4 → JUnit 5
terrain pytest2ut <source>     # pytest → unittest
terrain ut2pytest <source>     # unittest → pytest
terrain rspec2mini <source>    # RSpec → Minitest
terrain mini2rspec <source>    # Minitest → RSpec
terrain pup2pw <source>        # Puppeteer → Playwright
terrain wdio2pw <source>       # WebdriverIO → Playwright
```
...
```bash
terrain convert ./tests/ --from jest --to vitest --dry-run
# Output:
...
These patterns produce TERRAIN-TODO comments rather than broken output. Each has a unique ID for tracking.
