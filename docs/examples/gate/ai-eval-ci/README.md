...
- **Doesn't ship API keys.** `OPENAI_API_KEY` is consumed by
  ...
  [`SECURITY-DATA-HANDLING.md`](../../../../SECURITY-DATA-HANDLING.md).
- **Doesn't sandbox eval execution.** Sandboxing is future work. If your prompts include tool-call shapes that touch filesystem or network, the sandbox is the eval framework's responsibility in 0.4.0.
