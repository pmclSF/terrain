...
Every `terrain analyze` invocation runs a deterministic pipeline:
...
Every measurement produces a value, a band (strong/moderate/weak/elevated/critical), evidence strength, an explanation, and known limitations. Terrain never pretends to know more than it does.

### Portfolio Intelligence

`terrain portfolio` treats your test suite as an investment portfolio:
...
`terrain impact` integrates with git to assess which changed files are tested, which are not, and where test gaps exist in the current diff.
...
`terrain migration readiness` evaluates how prepared your codebase is for a framework migration:
...
Save snapshots with `terrain analyze --write-snapshot`, then compare them:

```
terrain compare
terrain compare --from baseline.json --to current.json
```

Trend highlights surface in `terrain summary` automatically when prior snapshots exist.

### Policy Governance

Define rules in `.terrain/policy.yaml` and enforce them:

```
terrain policy check
```

Exit code 2 on violations, suitable for CI gates.
...
Terrain tracks test identity across snapshots using a two-phase approach:
...
Lifecycle analysis is the foundation for stability classification and trend tracking. It enables Terrain to tell you "this test was renamed" rather than "a test was removed and a new one was added."

### Historical Stability Classes

Given 3+ snapshots, Terrain classifies each test into one of 7 stability classes:
...
Terrain detects 4 kinds of test suppression:
...
Terrain detects 5 types of common-cause clusters where shared dependencies drive broad test problems:
...
The `terrain pr` command performs change-scoped analysis against a git diff:
...
The `terrain show` command provides entity-level drill-downs:

```
terrain show test src/__tests__/auth.test.js   # Test file details with signals
terrain show unit AuthService                   # Code unit coverage and ownership
terrain show owner team-platform                # Owner portfolio with signals
terrain show finding untested_export            # Finding details with actions
```
...
The `terrain pr` command supports multiple output formats:
...
- **Protective test selection**: `terrain select-tests` recommends a focused test set for a change, with selection reasoning and gap reporting
...
- **PR/change-scoped workflows**: `terrain pr` with markdown, CI annotation, and concise comment output formats
- **Entity drill-down**: `terrain show` for test files, code units, owners, and findings

## Commands Reference

| Command | Description |
|---------|-------------|
| `terrain analyze` | Full test suite analysis with human-readable output |
| `terrain summary` | Executive summary with posture, risk areas, trends, and benchmark readiness |
...
| `terrain impact` | Impact analysis for changed code against git diff |
| `terrain impact --show graph` | View impact graph edges for impacted units |
| `terrain impact --show selected` | View protective test set with selection reasoning |
| `terrain select-tests` | Recommend protective test set for a change |
| `terrain metrics` | Aggregate metrics scorecard |
| `terrain migration readiness` | Migration readiness assessment |
| `terrain migration blockers` | List migration blockers by type and area |
| `terrain migration preview` | Preview migration for a file or scope |
...
| `terrain pr` | PR-scoped analysis with format options (markdown, annotation, comment) |
| `terrain show test <path>` | Drill into a specific test file |
| `terrain show unit <name>` | Drill into a specific code unit |
| `terrain show owner <name>` | Drill into an owner's portfolio |
| `terrain show finding <type>` | Drill into a finding type |
...
terrain portfolio         # cost and leverage analysis
terrain impact            # what changed and is it tested?
```

## Verification System

Terrain includes a formal, layered verification system:
