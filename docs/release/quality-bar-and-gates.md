...
| `make calibrate` (load-bearing in 0.2) | Calibration corpus runner asserts every labeled fixture's expected signal still fires (no false negatives); empty-corpus bypass closed via `t.Fatalf` + `minFixtures=25` |
