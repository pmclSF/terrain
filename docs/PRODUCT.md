...
> *Status: v3-redesigned draft. Three adversarial-review passes complete (v1 doc review, v2 audit-driven, v3 technical-excellence with verifier). 43 confirmed findings addressed via structural redesign across §3 (third co-equal goal), §9 (rule demotions), §10 (enum trim + tunable blocks), §12 (LB framework redesign with LB-2a/b/c, LB-6 revised, LB-9 per-phase, LB-11 bidirectional, LB-12 senior-decision-maker), §13 (corpus size raised, panel cadence reduced, funding source pinned), §14 (bidirectional validation, third-party perf benchmark), §16 (dependency-order spine, CLI surface acknowledged net-new, SARIF, headline-case test, gauntlet demoted, MCP spec pinned, VS Code Marketplace decision).*
...
Terrain unifies testing and development for AI systems. It treats unit tests, integration tests, end-to-end tests, ML model evaluations, data validation, and LLM eval scenarios as nodes in **a single dependency graph**, and surfaces failures in that graph as a **real CI gate** — not as a comment, dashboard, or email.
...
Terrain's detector engines analyze: **Python, TypeScript, JavaScript, Go, Java**. Code in other languages (Rust, Kotlin, Swift, C / C++, Ruby, etc.) is not currently analyzed — `terrain analyze` will emit findings on Python/JS/Go/Java build tooling within those repos but not on the primary language code. See §17 0.3.0 item 6 for the planned 0.3.0 decision (add Rust/Kotlin/Swift analyzers, or formalize the scope).

This scope clarification was added 2026-05-11 after the language-diversity corpus exercise confirmed empirically that the existing detector roster doesn't generate signals on Rust/Kotlin/Swift source.
...
**Acknowledged coupling risk:** carrying conversion's stability contract in the same release tag as the AI/ML gate couples their fates — a conversion-subsystem regression blocks the gate's release, and a gate-subsystem rename window forces conversion through it. This is accepted for 0.2.0 ship velocity; if it becomes a maintenance problem, splitting conversion into a separate binary/release cadence is a 0.3.0+ option per §17.
...
**Severity-field status (post-0.2.0 corpus calibration):** the 0.2.0 corpus measured severity vs. regression-PR co-occurrence and found the ladder has no predictive power: `critical` findings have 0.00× lift, `high` 1.05×, `medium` 0.74×, `low` 0.74×. 92.5% of findings emit at `medium`. The field is empirically decoration in 0.2.0. §17 0.3.0 item 3 re-bases severity on corpus lift + survival rate per-detector via the bundled `detector-evidence.json`. 0.2.0 callers should treat severity as effort-priority, not regression-risk.
...
- `coverage/no-fairness-eval` — depends on fairness-library recognition (Fairlearn, Evidently); §17 0.4.0+ work. Stays preview.
- `coverage/no-drift-monitor` — depends on drift-library recognition (Evidently, Alibi Detect, NannyML); §17 0.4.0+ work. Stays preview.
...
| 0.3.0 direction | Preview rules in drift, fairness, and data-validation categories become candidates for graduation as harness measurements clear |
| 0.4.0 direction | Lifecycle and registry-dependent rules become candidates as cross-system integrations land |
| 0.5.0 direction | Most of the catalog has had a graduation window |
...
The following rule IDs are reserved for post-0.2.0 releases (mentioned in §17 roadmap) and will not be reused for unrelated detectors:
...
- `performance/production-latency-regression` (0.5.0)
- `regression/production-error-spike` (0.5.0)
- `coverage/production-call-without-monitor` (0.5.0)
...
# but trimmed from v1 — they correspond to 0.5.0+/0.6.0+ integration roadmap (§17)
# and no 0.2.0 stable rule consumes them. They land in schema v2 when the rules
# that depend on them ship.
surfaces:
  summarizer:
...
| **Hand-labeled green / failure corpus** | ≥100 PRs per dogfood repo (mix of intended-green and seeded-failure), hand-labeled "should produce no findings" or "should produce findings X, Y, Z." Lives in `harness/corpora/<repo>/labels.yaml`. **Realistic labeling effort:** ~30 min per PR × 100 PRs × 5 repos ≈ 250 person-hours. **Labeling is done by the maintainer directly** rather than a multi-labeler consensus protocol — the project's solo-execution reality (§6) makes single-labeler the canonical mode at 0.2.0. The `terrain-corpus vote` / `adjudicate` subcommands (deferred to 0.3.0+ per §13) are designed for future multi-labeler scaling, not 0.2.0. Effort budget is real but bounded; spaced across the release cycle as Tier 4 work, not a single push. LB-3, LB-5 (FP), and LB-6 (recall) all measure against the labeled corpus. Corpus size of 100 (not 50) is the minimum needed for LB-5's Wilson 95% lower-bound at 5% to be statistically meaningful per §12. |
| **Runner** | Invokes Terrain on each PR (CI mode + CLI mode), captures all outputs |
| **Validators (automated)** | LB-1 schema check, LB-3 silence check, LB-4 reproduction-parity diff, LB-5 Wilson-95% FP-rate computation (per-rule per-repo with min-sample-size floor), LB-6 recall computation on seeded-failure corpus, LB-7 renderer-conformance check against `dorny/test-reporter` (GH), `mikepenz/action-junit-report` (GH), and GitLab's native JUnit consumer, LB-9 per-phase runtime measurement (graph / rules / render / total), LB-10 failure-mode probe, LB-11 bidirectional-cause-attribution scripted-PR runs (FE→AI and prompt-edit→FE), LB-12 senior-decision-maker comprehension panel |
...
- **Cadence:** once per release tag at minimum (0.2.0, 0.3.0, etc.). Spot checks per patch release only when a stable rule's detection mechanism changes. This is reduced from earlier draft's quarterly cadence — once-per-release at higher sample-size yields better statistical power and is operationally sustainable for an OSS project. Pre-1.0 the project publishes actual panel-run dates and outcomes in each release's readiness cards.
...
- **`cmd/terrain-corpus/`** — at 0.2.0 ships only two of its nine subcommands: `extract` (pulls candidate findings from snapshots into the labeling pipeline) and `gate` (enforces precision floors against the labeled corpus). The remaining 7 subcommands (`sample`, `vote`, `adjudicate`, `aggregate`, `diff`, `regen`, `mine`) are active-learning and corpus-scaling infrastructure that 0.2.0's hand-labeled-by-maintainer corpus doesn't need; they ship in 0.3.0+ when corpus scaling becomes necessary.
- **`cmd/terrain-precision/`** — at 0.2.0 ships `score` and `compare` only (compare detector configurations against labeled corpus, produce Wilson-95% precision metrics). `setup-fixtures` and `run` are full benchmark-harness orchestration that defers to 0.3.0+ alongside the active-learning subcommands.

Both tools are committed (no longer untracked), documented in `docs/HARNESS.md`, and treated as load-bearing infrastructure for LB-5 (false-positive budget measurement) — at the reduced 0.2.0 subcommand scope. They are *maintainer tools*, not adopter tools. The full 13-subcommand surface lands incrementally as corpus-scale needs justify; shipping all 13 at 0.2.0 was a premature commitment per the v3 review.
...
Each rule has a canonical page at `docs/rules/<rule-id>.md` (and at `terrain.dev/rules/<rule-id>` when the public docs site exists). The template serves both the developer hitting the finding and the senior decision-maker evaluating Terrain.
...
| Item | Load-bearing requirement |
|---|---|
| Cross-language API edges | TS/JS ↔ Python/Go/Java edges via OpenAPI, tRPC, gRPC `.proto`, GraphQL schemas, and HTTP-route inference. Audit 3b confirmed: zero such edges exist today. The headline FE→AI use case requires this. **Field-level narrowing primitive (per v3 review R3-I5):** route-level reachability alone over-selects impacted tests. Edges carry a `fields_read` set extracted from schema where available (OpenAPI body schema, tRPC type, gRPC message fields, GraphQL selection sets); rules like `regression/test-failed` filter impact selection to changes touching at least one field actually consumed downstream. For HTTP routes without schema, fall back to route-level (and document the limitation in the rule's FP-characterization page). |
...
| **VS Code extension at 0.2.0 — Marketplace publication, not sideload alpha** | Per v3-review correction: sideload-only VSIX won't reach the FE-developer persona the mission targets. Two viable paths and 0.2.0 commits to **path B**: (A) defer VS Code entirely to 0.3.0 with the LSP, or (B) ship a Marketplace-published alpha at 0.2.0 with the minimum capability set documented below. Path B chosen so the mission's "unifies testing *and development*" line has a real, discoverable referent at launch. **Minimum capability set:** (a) reads Terrain JUnit/JSON artifacts from disk or last-CI-run; (b) renders findings in the IDE Problems pane with severity-correct icons; (c) click-to-navigate to the cause-path primary location; (d) hover shows `Finding.short_message` + suggested action. *Not* in 0.2.0 alpha: inline squigglies, on-save analysis, refactoring actions, full LSP-server mode (those land in 0.3.0). Marketplace listing is clearly tagged "0.2.0 alpha — limited scope; full integration in 0.3.0." |
...
| Deferred to | Item |
|---|---|
| 0.3.0 | IDE full integration (LSP server, JetBrains, Neovim) — VS Code alpha ships at 0.2.0 as the seed |
| 0.3.0 | Additional language support beyond Go/JS/TS/Python/Java (Ruby, Rust, Kotlin, Swift, etc.) |
| 0.4.0+ | Marketplace Action publication (GitHub Actions Marketplace); Claude Skill / OpenAI listings as packaged wrappers around the MCP server. *Rationale for deferring even though MCP ships at 0.2.0:* marketplace listings are public-facing commitments to a specific UX surface; we want the MCP server to harden through 0.2.0 release and a release cycle of adopter use before locking down vendor-specific wrappers. Listings *could* ship sooner technically; the deferral is deliberate quality discipline, not engineering blocker. |
| 0.4.0+ | Polyrepo support — cross-repo edges. 0.2.0 analyzes a single repo; adopters with split FE/BE repos run Terrain on each independently. |
| 0.4.0+ | Additional eval framework adapters (Evidently, deepchecks, Fairlearn, NannyML) — the five must-ship adapters are the initial set |
| 0.5.0+ | Intra-procedural data-flow tracing; additional registry integrations (Comet, Neptune, ClearML) |
...
| Binary | Purpose |
|---|---|
| `cmd/terrain-corpus` | Calibration corpus management (nine subcommands per §13) |
...
## 17. Beyond 0.2.0 — phased roadmap

0.2.0 ships the unified-graph product end-to-end. Subsequent releases are *additive* — they extend coverage, add surfaces, graduate preview rules, broaden ecosystem reach. None of them are foundational architecture work 0.2.0 deferred.

Each phase is defined by *scope*, not by date. A release within a phase ships when its scope items meet the LB quality bars. Rules graduate from preview to stable when their LB-2 and LB-5 measurements clear the target bar on the dogfood repos. Phase boundaries describe the order in which capability lands, not a calendar.

### Integration philosophy (governs all post-0.2.0 work)
...
### 0.3.0 — Full IDE integration and developer reach

The Tier 5 corpus-driven quality work (formerly contemplated for 0.3.0) was promoted to must-ship for 0.2.0 — see §16. 0.3.0 therefore returns to its original additive scope:

- LSP server with full feature parity (CI-surface diagnostic fidelity in-IDE; the VS Code extension alpha that ships at 0.2.0 is the seed).
- Editor extensions for JetBrains, Neovim, Helix.
- Additional language support: Ruby, Rust, Kotlin (closes the Tier 5.6 scoping decision if adopter demand materializes).
- Preview-to-stable rule graduations as harness measurements clear (now driven by corpus PR-lift, not just precision).
- Calibration corpus continuous-improvement loop: `terrain-corpus drift-check` runs each release; precision-floor regressions > 5pp fail the release.

### 0.4.0 — Polyrepo, marketplace, deeper data + deployment integrations
- Cross-repo edges (polyrepo case): adopters with FE/BE in separate repos run Terrain across both with a shared graph
- Marketplace Action publication (`pmclSF/terrain-action@v1`)
- Claude Skill listing (wraps the MCP server)
- OpenAI / Apps SDK integration (wraps the MCP server)
- Additional eval framework adapters: Evidently, deepchecks, Fairlearn, NannyML
- **Deeper dbt integration** (extends the 0.2.0 manifest.json parsing): ingest `dbt test` results as Terrain findings — so a failed dbt test becomes a Terrain CI failure in the same test pipeline as other findings. Full lineage edges from dbt models to dependent code (not just model-to-model). `data/*` rules consume dbt-test failures. Boundary: 0.2.0 reads dbt's *structure* (manifest); 0.4.0 reads dbt's *runtime results* (test outcomes).
- **GraphQL ecosystem integration:** beyond schema parsing (in 0.2.0), ingest Apollo / Hasura / GraphQL-test-suite results; trace queries to resolvers as cross-language edges.
- **Test-execution platform ingestion:** Cypress Cloud, Playwright Cloud, BrowserStack, Sauce Labs — read run results, surface in the gate.

### 0.5.0 — Observability, production-aware testing, deep ML platform integration
- **Observability tool integration:** Honeycomb, Datadog, New Relic, Sentry, Grafana, Prometheus. Ingest production metrics (latency, error rate, traces) per surface. New rules: `performance/production-latency-regression`, `regression/production-error-spike`, `coverage/production-call-without-monitor`. The gate becomes informed by *production reality*, not just test-time signals.
- **Deployment orchestrator integration:** Argo CD, Spinnaker, Flagger pre-deploy hooks. Terrain runs as a deploy-gate, not just a merge-gate.
- **Data-observability integration:** Monte Carlo, Bigeye, Soda, Anomalo, Datafold. Data-quality signals flow into `data/*` rules.
- **Coverage data ingestion:** Codecov, Coveralls. Production coverage signal enriches `coverage/*` rules beyond structural graph traversal.
- **Deep ML platform integration** (beyond the 0.2.0 registry-awareness layer):
  - **SageMaker:** Pipelines (workflow orchestration as graph edges), Model Monitor (drift signals), Clarify (fairness signals), Endpoints (production-deployment awareness).
  - **Vertex AI:** Pipelines, Model Monitoring, Explainable AI, Endpoints.
  - **Databricks ML:** MLflow-hosted-in-Databricks, Unity Catalog model lineage.
  - **Azure ML:** Pipelines and Model Registry.
- **LLM-observability integration:** Langfuse, Helicone, Arize Phoenix; emerging standards OpenLLMetry, OpenInference.
- **Cost tracking integration:** Vantage, CloudZero, OpenCost — for `performance/cost-regression` rule.
- **Issue-tracker integration:** Linear, Jira — for "create issue from Terrain finding" flow. Configurable per-rule.

### 0.6.0+ — Polish, ecosystem integrations, scale
- Intra-procedural data-flow tracing (deeper cause attribution)
...
- **Feature-store integration:** Feast, Tecton, Hopsworks, Featureform. Feature-pipeline lineage from store to model surface.
- **Vector-database integration:** Pinecone, Weaviate, Chroma, Qdrant, Milvus, pgvector — for RAG evaluation and retrieval-quality rules.
- **ML governance integration:** Fiddler, Arthur, WhyLabs, Truera.
- **Notebook environment integration:** Jupyter, Hex, Deepnote, nbval, testbook — surface evals defined in notebooks.
- **Additional pipeline orchestrators:** Kubeflow, Metaflow, ZenML, Flyte.
- **Visual / UI testing integration:** Percy, Chromatic, Applitools — visual regression as `regression/*` rules.
- **Contract testing integration:** Pact (consumer-driven contracts), Spectral (OpenAPI linting).
- **Notifications (configurable per-rule, on error only):** Slack, MS Teams, PagerDuty. Deliberate non-default — silence-on-green principle holds; notifications are opt-in per-rule for adopters who want critical-only paging.
- Continued preview-to-stable graduations
- Performance and scale work for very large monorepos (>100k files)
- Additional CI platform support: Bitbucket Pipelines, Jenkins, Azure Pipelines, Buildkite, TeamCity.

### Out-of-scope integrations (explicit non-goals across all phases)
