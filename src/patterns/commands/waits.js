...
export const waitPatterns = {
  cypress: {
    ...
    generators: {
      ...
      WAIT_FOR_ELEMENT: (sel, timeout) =>
        `cy.get(${sel}, { timeout: ${timeout} })`,
      WAIT_FOR_VISIBLE: (sel, timeout) =>
        `cy.get(${sel}, { timeout: ${timeout} }).should('be.visible')`,
      WAIT_FOR_HIDDEN: (sel, timeout) =>
        `cy.get(${sel}, { timeout: ${timeout} }).should('not.be.visible')`,
      WAIT_FOR_ENABLED: (sel, timeout) =>
        `cy.get(${sel}, { timeout: ${timeout} }).should('be.enabled')`,
      WAIT_FOR_TEXT: (sel, text, timeout) =>
        `cy.get(${sel}, { timeout: ${timeout} }).should('contain', ${text})`,
  ...
  playwright: {
    ...
    generators: {
      ...
      WAIT_FOR_ALIAS: (_alias) =>
        `await page.waitForResponse(response => response.url().includes('${_alias}'))`,
      INTERCEPT_AS: (route, _alias) =>
      ...
      WAIT_FOR_VISIBLE: (sel, timeout) =>
        timeout
          ? `await page.waitForSelector(${sel}, { state: 'visible', timeout: ${timeout} })`
          : `await page.waitForSelector(${sel}, { state: 'visible' })`,
      WAIT_FOR_HIDDEN: (sel, timeout) =>
        timeout
          ? `await page.waitForSelector(${sel}, { state: 'hidden', timeout: ${timeout} })`
          : `await page.waitForSelector(${sel}, { state: 'hidden' })`,
      WAIT_FOR_ATTACHED: (sel) =>
        `await page.waitForSelector(${sel}, { state: 'attached' })`,
      WAIT_FOR_DETACHED: (sel) =>
        `await page.waitForSelector(${sel}, { state: 'detached' })`,
      WAIT_FOR_ENABLED: (sel, _timeout) =>
        `await page.locator(${sel}).waitFor({ state: 'visible' }); await expect(page.locator(${sel})).toBeEnabled()`,
      WAIT_FOR_TEXT: (sel, text, timeout) =>
        `await expect(page.locator(${sel})).toContainText(${text}, { timeout: ${timeout || 30000} })`,
  ...
  selenium: {
    ...
    generators: {
      ...
      WAIT_FOR_ALIAS: (_alias) =>
        "// Selenium doesn't have alias waits - use explicit wait",
      INTERCEPT_AS: () =>
        "// Selenium doesn't have built-in network interception",
      WAIT_FOR_ELEMENT: (sel, timeout) =>
        `await driver.wait(until.elementLocated(${sel}), ${timeout || 10000})`,
      WAIT_FOR_VISIBLE: (el, timeout) =>
        `await driver.wait(until.elementIsVisible(${el}), ${timeout || 10000})`,
      WAIT_FOR_HIDDEN: (el, timeout) =>
        `await driver.wait(until.elementIsNotVisible(${el}), ${timeout || 10000})`,
      WAIT_FOR_ATTACHED: (sel) =>
        `await driver.wait(until.elementLocated(${sel}), 10000)`,
      WAIT_FOR_DETACHED: (el) =>
        `await driver.wait(until.stalenessOf(${el}), 10000)`,
      WAIT_FOR_ENABLED: (el, timeout) =>
        `await driver.wait(until.elementIsEnabled(${el}), ${timeout || 10000})`,
      WAIT_FOR_TEXT: (el, text, timeout) =>
        `await driver.wait(until.elementTextContains(${el}, ${text}), ${timeout || 10000})`,
      WAIT_FOR_URL: (url) =>
      ...
      WAIT_FOR_FUNCTION: (fn) =>
        `await driver.wait(async () => await driver.executeScript(${fn}), 10000)`,
...
export const directMappings = {
  ...
  "cypress-selenium": {
    ...
    "cy\\.wait\\(['\"]@([^'\"]+)['\"]\\)":
      "// Selenium: use explicit wait for network response",
  },
