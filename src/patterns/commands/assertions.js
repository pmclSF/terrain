...
export const assertionPatterns = {
  ...
  playwright: {
    ...
    generators: {
      ...
      ASSERT_CONTAINS: (el, text) =>
        `await expect(${el}).toContainText(${text})`,
      ASSERT_NOT_CONTAINS: (el, text) =>
        `await expect(${el}).not.toContainText(${text})`,
      ...
      ASSERT_ATTRIBUTE: (el, attr, value) =>
        `await expect(${el}).toHaveAttribute(${attr}, ${value})`,
      ASSERT_HAS_ATTRIBUTE: (el, attr) =>
        `await expect(${el}).toHaveAttribute(${attr})`,
      ASSERT_CLASS: (el, cls) => `await expect(${el}).toHaveClass(${cls})`,
      ASSERT_NOT_CLASS: (el, cls) =>
        `await expect(${el}).not.toHaveClass(${cls})`,
      ASSERT_CSS: (el, prop, value) =>
        `await expect(${el}).toHaveCSS(${prop}, ${value})`,
      ...
      ASSERT_COUNT_GT: (el, n) =>
        `expect(await ${el}.count()).toBeGreaterThan(${n})`,
      ASSERT_COUNT_GTE: (el, n) =>
        `expect(await ${el}.count()).toBeGreaterThanOrEqual(${n})`,
      ASSERT_COUNT_LT: (el, n) =>
        `expect(await ${el}.count()).toBeLessThan(${n})`,
      ASSERT_COUNT_LTE: (el, n) =>
        `expect(await ${el}.count()).toBeLessThanOrEqual(${n})`,
  ...
  selenium: {
    ...
    generators: {
      ...
      ASSERT_VALUE: (el, value) =>
        `expect(await ${el}.getAttribute("value")).toBe(${value})`,
      ASSERT_ATTRIBUTE: (el, attr, value) =>
        `expect(await ${el}.getAttribute(${attr})).toBe(${value})`,
      ASSERT_HAS_ATTRIBUTE: (el, attr) =>
        `expect(await ${el}.getAttribute(${attr})).not.toBeNull()`,
      ASSERT_CLASS: (el, cls) =>
        `expect(await ${el}.getAttribute("class")).toContain(${cls})`,
      ASSERT_NOT_CLASS: (el, cls) =>
        `expect(await ${el}.getAttribute("class")).not.toContain(${cls})`,
      ASSERT_CSS: (el, prop, value) =>
        `expect(await ${el}.getCssValue(${prop})).toBe(${value})`,
      ASSERT_ID: (el, id) =>
        `expect(await ${el}.getAttribute("id")).toBe(${id})`,
      ASSERT_CHECKED: (el) => `expect(await ${el}.isSelected()).toBe(true)`,
      ASSERT_NOT_CHECKED: (el) =>
      ...
      ASSERT_FOCUSED: (el) =>
        `expect(await driver.switchTo().activeElement()).toEqual(${el})`,
      ASSERT_SELECTED: (el) => `expect(await ${el}.isSelected()).toBe(true)`,
      ASSERT_COUNT: (els, n) => `expect(${els}.length).toBe(${n})`,
      ASSERT_COUNT_GT: (els, n) =>
        `expect(${els}.length).toBeGreaterThan(${n})`,
      ASSERT_COUNT_GTE: (els, n) =>
        `expect(${els}.length).toBeGreaterThanOrEqual(${n})`,
      ASSERT_COUNT_LT: (els, n) => `expect(${els}.length).toBeLessThan(${n})`,
      ASSERT_COUNT_LTE: (els, n) =>
        `expect(${els}.length).toBeLessThanOrEqual(${n})`,
      ASSERT_URL_CONTAINS: (text) =>
        `expect(await driver.getCurrentUrl()).toContain(${text})`,
      ASSERT_URL_EQUALS: (url) =>
