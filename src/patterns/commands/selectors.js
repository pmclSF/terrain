...
export const selectorPatterns = {
  ...
  playwright: {
    ...
    generators: {
      ...
      'SELECT_BY_TEXT_IN': (sel, text) => `page.locator(${sel}).filter({ hasText: ${text} })`,
      'SELECT_BY_ROLE': (role) => `page.getByRole(${role})`,
      'SELECT_BY_LABEL': (label) => `page.getByLabel(${label})`,
      'SELECT_BY_PLACEHOLDER': (placeholder) => `page.getByPlaceholder(${placeholder})`,
  ...
  selenium: {
    patterns: {
      ...
      'await driver\\.findElement\\(By\\.xpath\\(([^)]+)\\)\\)': 'SELECT_XPATH($1)',
      'await driver\\.findElement\\(By\\.id\\(([^)]+)\\)\\)': 'SELECT_BY_ID($1)',
      'await driver\\.findElement\\(By\\.name\\(([^)]+)\\)\\)': 'SELECT_BY_NAME($1)',
      'await driver\\.findElement\\(By\\.className\\(([^)]+)\\)\\)': 'SELECT_BY_CLASS($1)',
      'await driver\\.findElement\\(By\\.tagName\\(([^)]+)\\)\\)': 'SELECT_BY_TAG($1)',
      'await driver\\.findElement\\(By\\.linkText\\(([^)]+)\\)\\)': 'SELECT_BY_LINK_TEXT($1)',
      'await driver\\.findElement\\(By\\.partialLinkText\\(([^)]+)\\)\\)': 'SELECT_BY_PARTIAL_LINK_TEXT($1)',
    ...
    generators: {
      ...
      'SELECT_BY_CLASS': (className) => `await driver.findElement(By.className(${className}))`,
      'SELECT_BY_TAG': (tag) => `await driver.findElement(By.tagName(${tag}))`,
      'SELECT_BY_LINK_TEXT': (text) => `await driver.findElement(By.linkText(${text}))`,
      'SELECT_BY_PARTIAL_LINK_TEXT': (text) => `await driver.findElement(By.partialLinkText(${text}))`,
      'SELECT_ALL': (sel) => `await driver.findElements(${sel})`,
      'FIND': (sel) => `.findElement(${sel})`,
      'SELECT_BY_TEXT': (text) => `await driver.findElement(By.xpath(\`//*[contains(text(),${text})]\`))`,
      'SELECT_BY_TEXT_IN': (sel, text) => `await driver.findElement(By.xpath(\`${sel}[contains(text(),${text})]\`))`,
      'FIRST': () => '[0]',
      'LAST': () => '.slice(-1)[0]',
      'NTH': (n) => `[${n}]`,
      'PARENT': () => '.findElement(By.xpath(".."))',
      'PARENTS': (sel) => `.findElement(By.xpath("ancestor::${sel}"))`,
      'CHILDREN': (sel) => sel ? `.findElements(By.css(${sel}))` : '.findElements(By.xpath("child::*"))',
      'SIBLINGS': () => '.findElements(By.xpath("following-sibling::* | preceding-sibling::*"))',
